...
import {
  getTotalItemCount as getInitialSetupTotal,
  getCompletedItemCount as getInitialSetupCompleted,
  ...
  return (
    <div className="flex min-h-screen bg-[#F0FAFA]">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        {/* Initial Setup card — only on New Realtor® level */}
        {activeLevel.id === "new" && (() => {
          const setupTotal = getInitialSetupTotal();
          const setupDone  = getInitialSetupCompleted(Array.from(completed));
          const setupPct   = setupTotal > 0 ? Math.round((setupDone / setupTotal) * 100) : 0;
          return (
            <div className="bg-white border-2 border-[#0D5C63] rounded-xl p-5 mb-4">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0 flex-1">
                  <h2 className="text-lg font-bold text-[#0D5C63] mb-0.5">📋 Initial Setup</h2>
                  <p className="text-sm text-slate-600 mb-3">Your guided onboarding checklist</p>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-medium text-slate-700">
                      {setupDone} of {setupTotal} complete
                    </span>
                    <span className="text-xs font-semibold text-[#0D5C63]">{setupPct}%</span>
                  </div>
                ...
                <Link
                  ...
                  className="bg-[#FF6B35] text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:opacity-90 transition-opacity whitespace-nowrap shrink-0"
                >
                  Open Initial Setup →
                </Link>
              </div>
